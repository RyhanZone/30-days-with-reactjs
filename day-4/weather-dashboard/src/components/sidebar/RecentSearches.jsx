    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2.5 overflow-y-auto pr-1 scrollbar-thin">
        {searches.map((item) => (
          ...
          >
            <div>
              <p className="text-sm font-semibold text-slate-100">{item.city}</p>
              <p className="text-xs text-slate-400">{item.country}</p>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold text-slate-100">{item.temp}°C</span>
              <WeatherIcon icon={item.icon} className="h-5 w-5" />
            </div>
          </button>
        ))}
      </div>
